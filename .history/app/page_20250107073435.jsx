...
const Home = () => {
  ...
  const downloadPDF = async () => {
    try {
      const response = await fetch("/app/api/generate-pdf"); // Llamada a la API
      if (!response.ok) {
        throw new Error("No se pudo generar el PDF.");
      }
      const blob = await response.blob(); // Obtiene el archivo PDF como un blob
      const url = window.URL.createObjectURL(blob); // Crea una URL para el archivo

      // Crea un enlace para descargar el archivo
      const link = document.createElement("a");
      link.href = url;
      link.download = "MiPortafolio.pdf"; // Nombre del archivo descargado
        ...
        <div className="flex flex-col xl:flex-row items-center justify-between xl:pt-8 xl:pb-24">
          ...
          <div className="text-center xl:text-left order-2 xl:order-none">
            ...
            <h1 className="h1 mb-6">
              Hola soy <br /> <span className="text-accent">Daniel Lazaro Hernandez</span>
            </h1>
